...
/*	if(fixedeye || tempfixeye)
	...
	if(loc == NewLoc)
		...
		if(wear_armor && istype(wear_armor, /item/clothing)) //CC Edit: More powder flask shitcode runtimes
			if(mobility_flags & MOBILITY_STAND)
				wear_armor.step_action()

		if(wear_shirt && istype(wear_shirt, /item/clothing)) //CC Edit: More powder flask shitcode runtimes
			if(mobility_flags & MOBILITY_STAND)
				wear_shirt.step_action()

		if(cloak && istype(cloak, /item/clothing)) //CC Edit: More powder flask shitcode runtimes
			if(mobility_flags & MOBILITY_STAND)
